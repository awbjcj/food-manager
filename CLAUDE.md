...
Quota is pooled by household. Receipt ingest hard-blocks at exhaustion; other
LLM-backed paths degrade to non-LLM behavior. `admit()` runs before work and
`commit()` records attempts/cost afterward, including while enforcement is dark.
Stars payment validation binds the payer, SKU, amount, recurrence, and household;
the ledger row and entitlement update commit in one transaction.
...
### Affinity + recipe media (v5.3)

`app/cook/affinity.py` turns recent 👍/👎 feedback (`app/cook/feedback.py`,
`FeedbackSignal`) into a taste score consumed by both `/cook` and `/plan`
ranking: `list_recent_signals` reads the household's most recent 50 signals
(`SIGNAL_WINDOW`), `affinity(cuisine, ingredient_names, signals)` averages
per-signal similarity into a `0.05-1.0` score — `0.5` cold-start with no
signals, and dislikes are soft-suppressed (clamped to a `0.05` floor, never 0) so a single bad rating can never hard-exclude a cuisine/ingredient
