...
def _sourced(title, *, ingredients, external_id, health=50, deliciousness=0.5):
    return SourcedRecipe(
        ...
        nutrition=NutritionScore(
            health_score=health, effort="easy", est_minutes=20, rationale="x"
        ),
...
@pytest.mark.asyncio
async def test_plan_unexpected_failure_shows_error_not_a_crash(
    ...
    monkeypatch.setattr(plan_handlers, "build_plan", failing_build_plan)
    msg = _msg("/plan 3")
    await bot_mod.handle_plan(
        msg,
        session_factory=session_factory,
...
async def test_plan_tiny_pantry_replies_not_enough(session_factory, monkeypatch):
    ...
    ack = msg.answer.return_value
    text = (
        ack.edit_text.await_args.args[0]
        if ack.edit_text.await_args
...
async def test_plan_swap_replaces_entry_and_rerenders(session_factory, monkeypatch):
    monkeypatch.setattr(handler_support, "ALLOWED_TELEGRAM_USER_ID", 1)
    plan_id = await _build_plan_for_callbacks(
        session_factory, days_specs=_specs3(), pages=_pages3()
    )

    fresh_source = FakeRecipeSource(
        [[_sourced("Fresh Yogurt", ingredients=["yogurt"], external_id="Z")]]
    )
...
@pytest.mark.asyncio
async def test_plan_swap_no_result_leaves_message_with_notice(
    session_factory, monkeypatch
):
    monkeypatch.setattr(handler_support, "ALLOWED_TELEGRAM_USER_ID", 1)
    plan_id = await _build_plan_for_callbacks(
...
@pytest.mark.asyncio
async def test_plan_shop_adds_union_then_second_tap_adds_zero(
    session_factory, monkeypatch
):
    monkeypatch.setattr(handler_support, "ALLOWED_TELEGRAM_USER_ID", 1)
    ...
    plan_id = await _build_plan_for_callbacks(
        session_factory, days_specs=_specs3(), pages=pages
    )
...
@pytest.mark.asyncio
async def test_plan_cancel_is_terminal_and_blocks_further_callbacks(
    session_factory, monkeypatch
):
    monkeypatch.setattr(handler_support, "ALLOWED_TELEGRAM_USER_ID", 1)
    plan_id = await _build_plan_for_callbacks(
